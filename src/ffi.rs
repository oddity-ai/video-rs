...
/// This function is similar to the existing bindings in ffmpeg-next
/// like `output` and `output_as`, but does not assume that it is
/// opening a file-like context. Instead, it opens a raw output,
/// without a file attached.
///
/// Combined with the `output_raw_buf_start` and `output_raw_buf_end`
/// functions, this can be used to write to a buffer instead of a file.
/// 
/// # Arguments
/// 
...
/// This function initializes a dynamic buffer and inserts it into an
/// output context to allow a write to happen. Afterwards, the callee
/// can use `output_raw_buf_end` to retrieve what was written.
/// 
/// # Arguments
/// 
...
pub fn output_raw_buf_start(output: &mut Output) {
  unsafe {
    // Here we initialize a raw pointer (mutable) as nullptr initially.
    // We then call the `avio_open_dyn_buf` which expects a ptr ptr, and
    // place the result in p. In case of success, we override the `pb`
    // pointer inside the output context to point to the dyn buf.
    let mut p: *mut AVIOContext = ptr::null_mut();
...
/// This function cleans up the dynamic buffer used for the write and
/// returns the buffer as a vector of bytes.
/// 
/// # Arguments
/// 
...
pub fn output_raw_buf_end(output: &mut Output) -> Vec<u8> {
  unsafe {
    // First, we acquire a raw pointer to the AVIOContext in the `pb`
    // field of the output context. We stored the dyn buf there when
    // we called `output_raw_buf_start`. Secondly, the `close_dyn_buf`
    // function will place a pointer to the starting address of the
    // buffer in `buffer_raw` through a ptr ptr. It also returns the
    // size of that buffer.
    let output_pb = ((*output.as_mut_ptr()).pb) as *mut AVIOContext;
    let mut buffer_raw: *mut u8 = ptr::null_mut();
    let buffer_size = avio_close_dyn_buf(
      output_pb,
      (&mut buffer_raw) as *mut *mut u8) as usize;

    // Reset the `pb` field or `avformat_close` will try to free it!
    ((*output.as_mut_ptr()).pb) = ptr::null_mut::<AVIOContext>();
    ...
    av_free(buffer_raw as *mut c_void);

    buffer
  }
}

/// This function initializes an IO context for the `Output` that packetizes
/// individual writes. Each write is pushed onto a packet buffer (a collection
/// of buffers, each being a packet).
/// 
/// The callee must invoke `output_raw_packetized_buf_end` soon after calling
/// this function. The `Vec` pointed to by `packet_buffer` must live between
/// invocation of this function and `output_raw_packetized_buf_end`!
/// 
/// Not calling `output_raw_packetized_buf_end` after calling this function
/// will result in memory leaking.
/// 
/// # Arguments
/// 
/// * `output` - Output context to start write on.
...
pub fn output_raw_packetized_buf_start(
  ...
  unsafe {
    ...
    let mut io: *mut AVIOContext =
      avio_alloc_context(
        buffer,
        max_packet_size.try_into().unwrap(),
        // Set stream to WRITE.
        1,
        // Pass on a pointer *UNSAFE* to the packet buffer, assuming
        // the packet buffer will live long enough.
        transmute(packet_buffer),
        // No `read_packet`.
        None,
    ...
    // Setting `max_packet_size` will let the underlying IO stream know
    // that this buffer must be treated as packetized.
    (*io).max_packet_size = max_packet_size
      .try_into()
      .unwrap();
...
/// This function cleans up the IO context used for packetized writing
/// created by `output_raw_packetized_buf_start`.
/// 
/// # Arguments
/// 
/// * `output` - Output context to end write on.
pub fn output_raw_packetized_buf_end(
  output: &mut Output,
) {
  unsafe {
    let output_pb = ((*output.as_mut_ptr()).pb) as *mut AVIOContext;
    ...
    // Note: No need for handling `opaque` as it is managed by Rust
    // code anyway and will be freed by it.

    // We do need to free the buffer itself though (we allocatd it
    // manually earlier).
    av_free((*output_pb).buffer as *mut c_void);
    // And deallocate the entire IO context.
    av_free(output_pb as *mut c_void);
...
/// 
/// For example: It is used to flush fragments when outputting fragmented
/// mp4 packets in combination with the `frag_custom` option.
/// 
/// # Arguments
/// 
...
/// Get the `time_base` field of an encoder. (Not natively supported in
/// the public API.)
/// 
/// # Arguments
/// 
...
pub fn get_encoder_time_base(encoder: &Video) -> Rational {
  unsafe {
    (*encoder.0.as_ptr()).time_base.into()
  }
}
...
/// A frame array is the `ndarray` version of `AVFrame`. It is 3-dimensional
/// array with dims `(H, W, C)` and type byte.
#[cfg(feature = "ndarray")]
...
/// # Arguments
/// 
/// * `frame_array` - Video frame to convert. The frame format must be
///   `(H, W, C)`.
/// 
/// # Returns
/// 
...
#[cfg(feature = "ndarray")]
pub fn convert_ndarray_to_frame_rgb24(
  ...
  unsafe {
    ...
    // Temporary frame structure to place correctly formatted data
    // and linesize stuff in, which we'll copy later.
    let mut frame_tmp = Frame::empty();
    let frame_tmp_ptr = frame_tmp.as_mut_ptr();

    // This does not copy the data, but it sets the `frame_tmp` data
    // and linesize pointers correctly.
...
pub fn convert_frame_to_ndarray_rgb24(
  ...
  unsafe {
    ...
    let frame_format = transmute::<c_int, AVPixelFormat>((*frame_ptr).format);
    assert_eq!(frame_format, AVPixelFormat::AV_PIX_FMT_RGB24);

    let mut frame_array = FrameArray::default((
...
/// Retrieve a reference to the extradata bytes in codec parameters of
/// an output stream.
/// 
/// # Arguments
/// 
...
pub fn extradata<'params>(
  output: &'params Output,
  stream_index: usize,
) -> Result<&'params [u8], Error> {
...
/// Whether or not the output format context is configured to use H.264
/// packetization mode 0.
/// 
/// # Arguments
/// 
/// * `output` - Output format context.
pub fn rtp_h264_mode_0(
  output: &Output,
) -> bool {
  unsafe {
    av_opt_flag_is_set(
...
pub fn rtp_seq_and_timestamp(
  output: &Output,
) -> (u16, u32) {
  unsafe {
    let rtp_mux_context =
      &*((*output.as_ptr()).priv_data as *const RTPMuxContext);
...
/// 
/// A media entry will be created for each stream in the output. This function
/// will take care of all details, such as setting the correct media attributes
/// needed by any SDP consumers.
/// 
/// # Arguments
/// 
...
pub fn sdp(
  ...
  unsafe {
    let mut buf: [c_char; BUF_SIZE as usize] = [0; BUF_SIZE as usize]; 
    let buf_ptr = &mut buf as *mut c_char;
    ...
    let ret = av_sdp_create(
      output_format_context_ptr,
      1,
      buf_ptr,
      BUF_SIZE);

    if ret == 0 {
      let sdp_c_str = CStr::from_ptr(buf_ptr);
...
/// Initialize the logging handler. This will redirect all ffmpeg logging
/// to the Rust `tracing` crate and any subscribers to it.
pub fn init_logging() {
  unsafe {
...
/// Passthrough function that is passed to `libavformat` in `avio_alloc_context`
/// and pushes buffers from a packetized stream onto the packet buffer held
/// in `opaque`.
extern "C" fn output_raw_buf_start_callback(
  ...
  unsafe {
    // Acquire a reference to the packet buffer transmuted from
    // the `opaque` gotten through `libavformat`.
    let packet_buffer: &mut Vec<Vec<u8>> = transmute(opaque);
...
fn log_filter_hacks(
  ...
  if line.find(HACK_1_PELCO_NEEDLE_1).is_some() &&
     line.find(HACK_1_PELCO_NEEDLE_2).is_some() {
    return false;
  }
