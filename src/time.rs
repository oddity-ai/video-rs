...
/// Represents a frame timestamp relative to the start of original stream. This can be either a
/// presentation timestamp (PTS), decoder timestamp (DTS) or even a duration.
#[derive(Clone, Debug)]
pub struct Time {
    time: Option<i64>,
...
impl Time {
    ...
    /// Create a new timestamp by its time value and time base in which the time is expressed. These
    /// two components are enough to rescale to a new time base.
    ///
    /// # Arguments
    ///
    /// * `time` - Relative time in `time_base` units.
    /// * `time_base` - Time base of source.
    pub(crate) fn new(time: Option<i64>, time_base: AvRational) -> Time {
        Self { time, time_base }
    }
    ...
    /// Convert to underlying time in `i64`. Assumes that the user knows the time base and applies
    /// it correctly.
...
impl From<Time> for Duration {
    ...
    fn from(timestamp: Time) -> Self {
        if let Some(offset) = timestamp.time {
            let micros = offset
                // By rescaling into 1/1 million, we're essentially acquiring the number of
                // microseconds that is the offset of this timestamp.
